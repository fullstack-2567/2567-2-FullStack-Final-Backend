...
export class ReportsService {
  ...
  async getUsersReport() {
    ...
    const userDataPromises = users.map(async (user) => {
      ...
      const coursesTaken = await this.enrollmentRepository.count({
        where: { userId: user.userId },
      });

      // Count completed courses
      const coursesCompleted = await this.enrollmentRepository.count({
        where: {
          userId: user.userId,
  ...
  async getContentsReport() {
    ...
    const contentDataPromises = contents.map(async (content) => {
      ...
      const studentsEnrolled = await this.enrollmentRepository.count({
        where: { contentId: content.contentId },
      });

      // Count students who completed the course
      const studentsCompleted = await this.enrollmentRepository.count({
        where: {
